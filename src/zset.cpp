...
/**
 * znode_new - Create a new ZNode with given name and score
 * @name: Name string (not necessarily null-terminated)
 * @len: Length of the name string
 * @score: Score value for sorting
 * 
 * Allocates and initializes a new ZNode with the specified name and score.
 * The name is copied into the flexible array member. The hash code is
 * precomputed for efficient hash table operations.
 * 
 * Returns: Pointer to the newly created ZNode
 * Note: Uses assert() for memory allocation failure - not suitable for production
 */
...
/**
 * znode_del - Delete a ZNode and free its memory
 * @node: ZNode to delete
 * 
 * Simple wrapper around free() for ZNode deallocation.
 */
...
/**
 * ZNode comparison function for AVL tree ordering
 * ===============================================
 * Nodes are ordered first by score, then by name for tie-breaking.
 * This ensures a stable, deterministic ordering even when scores are equal.
 */
static bool zless(
